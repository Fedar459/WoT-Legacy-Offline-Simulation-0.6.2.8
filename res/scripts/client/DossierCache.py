...
class DossierCache(object):
    ...
    def __init__(self, accountName):
        ...
        p = os.path
        import locale
        prefsFilePath = BigWorld.wg_getPreferencesFilePath().decode('utf-8').encode(locale.getpreferredencoding())
        self.__cacheDir = p.join(p.dirname(prefsFilePath), 'dossier_cache')
        _server = BigWorld.server() or 'offline'
        if _server is None:
            _server = "offline"
        self.__cacheFileName = p.join(self.__cacheDir, _server.replace(':', '#') + '#' + accountName + '.dat')
        self.__cache = {}
