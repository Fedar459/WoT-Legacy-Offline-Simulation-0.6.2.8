# STREAMING_CHUNK:Imports and basic configuration...
# -*- coding: utf-8 -*-
import BigWorld
import Math
import math
...
def _arena_vehicle_info_tuple(vehID, compactDescr, name, team, isAlive=True, isAvatarReady=True, accountDBID=0, prebattleID=0):
    """ClientArena.__vehicleInfoAsDict в 0.6.2.8: 10 полей, без isTeamKiller/actions."""
...
def _arena_name_to_id(arena_id):
    """Как ArenaType.init(): g_list[id] = name из scripts/arena_defs/_list_.xml."""
    import ArenaType
...
def _music_loading_event(MC):
    """Avatar.onBecomePlayer: MUSIC_EVENT_COMBAT_LOADING (в 0.6.2.8 нет MUSIC_EVENT_LOADING)."""
...
def _patch_offline_media_fixes():
    """БАГФИКС: собраны здесь, а не в оригинальных скриптах игры, специально -"""
    try:
    ...
    try:
        ...
        if not getattr(_BigWorld, '_offline_mem_hook_patched', False):
            ...
            _BigWorld.onMemoryCritical = _safe_onMemoryCritical
            try:
                _game.onMemoryCritical = _safe_onMemoryCritical
            except Exception:
                pass
...
# STREAMING_CHUNK:Offline battle chat (fake channels + bot phrases + player broadcast)...
...
def _generate_offline_chat_channels():
    """Регистрирует локальные (не серверные) каналы боевого чата."""
    try:
...
def _offline_post_chat_message(cid, originatorID, originatorName, text):
    """Кладёт сообщение прямо в окно чата боя и в историю канала, минуя"""
    try:
        dispatcher = _MessengerDispatcherMod.g_instance
        if dispatcher is None:
            return
...
def _offline_try_chat_shortcuts(avatar, isDown, key, mods):
    """F2–F7 / атака цели: штатный Avatar.handleKeyEvent в оффлайне не вызывается."""
    if not isDown:
        return False
...
def _weighted_choice(pairs):
    """Взвешенный случайный выбор без random.choices (тут Python 2)."""
...
# STREAMING_CHUNK:Random bot module loadout (chassis/engine/radio/fuelTank/turret+gun)...
def _randomize_bot_modules(botDescr):
    """Случайно расставляет боту исследованные модули вместо всегда стокового"""
    try:
        t = botDescr.type
...
# STREAMING_CHUNK:Defining maps and bots configs...
_BOT_TANK_LIST = [
...
def _build_map_spawns():
    """SpawnPoint UDOs from revivedoftanks041; old table is fallback."""
    result = {}
...
def _extract_team_bases_from_chunks(arena_id):
    """TeamBase UDO / Flagstaff из .chunk карты (как SpawnPoint в spawnpoints.py)."""
    found = []
    if not arena_id:
        return found
    try:
...
def _pair_bases_to_spawn_teams(extracted, arena_id):
    """Привязка найденных кругов к командам по центру SpawnPoint (тот же"""
...
_BOT_REVERSE_FUTILE_COOLDOWN = 2.0
...
_BOT_REVERSE_AFTER_BLOCKED = 0.8
...
# STREAMING_CHUNK:Utility functions for destructibles and heights...
...
def _scenery_teach_cache():
    ...
    try:
        ...
        def getDescIDByFilename(self, filename):
            ...
            if found is not None or not filename:
                return found
            found = original(self, _SCENERY_LOD.sub('/lod0/', filename))
            if found is not None:
                return found
            normalised = _SCENERY_LOD.sub('/lod0/', filename).replace('\\', '/').lower()
            for part in _SCENERY_NON_INTACT:
                if part in normalised:
                    return None
            ...
            if table is None:
                ...
                for name, descID in known.iteritems():
                    key = _scenery_path_suffix(_SCENERY_LOD.sub('/lod0/', name))
                    if key not in table:
                        table[key] = descID
                    elif table[key] != descID:
                        table[key] = None
                self._offline_bySuffix = table
            return table.get(_scenery_path_suffix(normalised))
...
def _scenery_run_over(vehicle, fromXZ, toXZ, elapsed):
    """revived run_over: энергия 0.5*m*v^2*0.00015, сегмент корпуса, v>=0.5."""
    ...
    for chunkID, chunk, slot in _scenery_candidates(minX, minZ, maxX, maxZ):
        ...
        try:
            ...
            kind = None
        if kind in (DESTR_TYPE_TREE, DESTR_TYPE_FALLING_ATOM, DESTR_TYPE_FRAGILE):
...
def _wall_mem_add(x, z):
    """Запомнить стену около (x, z). Возвращает True, если запись новая."""
    try:
        ...
        try:
            LOG_NOTE("[BATTLE][WALLMEM] wall remembered at (%.0f, %.0f), total %d" % (x, z, len(_G_WALL_MEM)))
        except Exception:
            pass
        return True
    except Exception:
...
def _wall_mem_near(x, z, radius=None):
    """Есть ли запомненная стена ближе radius к (x, z)."""
    try:
        if not _G_WALL_MEM:
            return False
        if radius is None:
...
def _wall_mem_seg_blocked(x0, z0, x1, z1, radius=None, samples=3):
    """Пересекает ли отрезок (x0,z0)-(x1,z1) окрестность запомненной стены."""
    try:
        if not _G_WALL_MEM:
            return False
        if radius is None:
...
def _bot_probe_hit_any_mask(spaceID, sx, sy, sz, ex, ey, ez):
    """Проба отрезка несколькими масками коллизии."""
    _hit128 = None
    try:
...
def _bot_see_invisible_wall(spaceID, x, y, z, yaw, dist=None):
    """Видит ли бот из (x,z) по курсу yaw невидимую стену."""
    try:
        if dist is None:
...
def _bot_trigger_unstuck_reverse(bot_veh, cur_x, cur_z, cur_yaw, reason):
    """Старт манёвра 'отъехать назад и развернуться'."""
    try:
        _side = -1 if _random.random() < 0.5 else 1
        try:
            _wx = cur_x + _math.sin(cur_yaw) * 7.0
            _wz = cur_z + _math.cos(cur_yaw) * 7.0
            _wall_mem_add(_wx, _wz)
        except Exception:
            pass
        try:
            bot_veh._ai_reverse_side = _side
        except Exception:
            pass
        try:
            bot_veh._ai_reverse_until = BigWorld.time() + _BOT_UNSTUCK_REVERSE_TIME
        except Exception:
            pass
        try:
            _turn_deg = _random.uniform(_BOT_UNSTUCK_TURN_DEG_MIN, _BOT_UNSTUCK_TURN_DEG_MAX)
            bot_veh._ai_post_reverse_turn = cur_yaw + _side * _math.radians(_turn_deg)
            bot_veh._ai_post_reverse_until = BigWorld.time() + _BOT_UNSTUCK_REVERSE_TIME + 2.0
        except Exception:
            pass
        try:
            LOG_NOTE("[BATTLE][NAV] bot %d unstuck: %s - reversing %.1fs then turning %s" % (
                getattr(bot_veh, 'id', -1), reason, _BOT_UNSTUCK_REVERSE_TIME,
...
def _destr_desc(spaceID, chunkID, itemIndex):
    """Описание объекта из AreaDestructibles.g_cache (тот же путь, что клиент)."""
    try:
        from AreaDestructibles import g_cache
...
def _destr_type_for_matKind(spaceID, chunkID, itemIndex):
    """Возвращает тип destructible-объекта (DESTR_TYPE_TREE/FALLING_ATOM/"""
    try:
...
def _is_protected_cap_scenery(spaceID, chunkID, itemIndex):
    """Флагшток/ёлка круга захвата: в клиенте это scenery, оффлайн ломал её как дерево."""
    filename = None
    try:
        filename = BigWorld.wg_getDestructibleFilename(spaceID, chunkID, itemIndex)
    except Exception:
...
def _seg_hit_is_ground(res, minPlaneY=0.40, tankY=None):
    """Горизонтальный ус: земля/кочка/склон, не стена дома."""
    try:
        if res is None:
            return False
        if res[1].y >= minPlaneY:
            return True
        if tankY is not None and abs(res[0].y - tankY) < 1.15:
            return True
    except Exception:
        pass
    return False
...
def _destr_whisker_blocks(spaceID, matKind, itemIndex, chunkID):
    """Тот же разбор hard/soft, что и в OfflineBattle._vehicle_try_break_destructible"""
...
def _destr_structure_intact(spaceID, res):
    """КОЛЛИЗИЯ ДОМОВ: True, если луч res упёрся в ещё ЦЕЛУЮ секцию здания"""
    try:
        if res is None:
            return False
        matKind = res[2]
...
def _measure_building_footprint(spaceID, wx, wz, probeH):
    """КОЛЛИЗИЯ ДОМОВ 2 - 'ровное поле по размерам дома': измеряет отпечаток"""
    try:
        pts = []
...
def _dispatch_destructible_hit(spaceID, matKind, itemIndex, chunkID, dirVec, isShotDamage=True):
    """Общая часть "какого типа объект и как его ломать" - раньше жила"""
    try:
        if matKind < _const.DESTRUCTIBLE_MATKINDS_MIN or matKind > _const.DESTRUCTIBLE_MATKINDS_MAX:
            return False
        ...
        if dtype == DESTR_TYPE_TREE or dtype == DESTR_TYPE_FALLING_ATOM:
            ...
            if matKind >= _const.DESTRUCTIBLE_MATKINDS_NORMAL_MIN and matKind <= _const.DESTRUCTIBLE_MATKINDS_NORMAL_MAX:
                data = encodeDestructibleModule(itemIndex, matKind, bool(isShotDamage))
...
def _try_destroy_destructible_at(spaceID, impactPoint, shotDir):
    """Разрушение по попаданию снаряда: сам ищет destructible-поверхность"""
    try:
...
def _shot_static_hit(spaceID, shotPos, farPoint, shotDir):
    """Тот же разбор static-луча, что у ботов в _botAITick: деревья/забор"""
...
def _spawn_ground_y(spaceID, x, z, hint_y):
    """Sit hull on terrain, not on tree tops / roofs (spawn-in-sky bug)."""
    try:
        hint = float(hint_y)
    except Exception:
        hint = 2.0
...
def _nav_ground_snap(spaceID, x, z, ref_y, band=3.0):
    """То же самое, что get_ground_height выше, но с одной важной поправкой."""
    try:
...
def _nav_ground_matkind(spaceID, x, z, ref_y, band=3.0):
    """Тот же самый узкополосный луч, что и у _nav_ground_snap чуть выше (см."""
    try:
        res = BigWorld.wg_collideSegment(spaceID, Math.Vector3(x, ref_y + band, z), Math.Vector3(x, ref_y - band, z), 128)
        if res is not None:
            return res[2]
...
def _water_point_hit(x, y, z):
    """Вода у гусениц: как ProjectileMover (waterY = p0.y - dist), только если"""
    try:
...
def _hull_over_water(spaceID, cx, cz, y, yaw, hw, hl):
    """Вода под любой точкой корпуса: центр + нос + корма + борта."""
    try:
        _sy = _math.sin(yaw)
        _cy = _math.cos(yaw)
        _ry = _math.cos(yaw)
...
def _ground_narrow(spaceID, x, z, ref_y, band=3.0):
    """Высота земли в узкой полосе вокруг ref_y, либо None (пустота/обрыв)."""
    try:
        res = BigWorld.wg_collideSegment(spaceID, Math.Vector3(x, ref_y + band, z), Math.Vector3(x, ref_y - band, z), 128)
...
def _carrying_corners_world(x, z, yaw, hw, hl):
    """4 угла гусениц из chassis topRightCarryingPoint — те же вершины, из"""
    sy = _math.sin(yaw)
    cy = _math.cos(yaw)
    rx = _math.cos(yaw)
    rz = -_math.sin(yaw)
...
def _carrying_ground_y(spaceID, x, z, ref_y, band=2.2):
    """Земля под точкой опоры фильтра. Бок кочки (крутая нормаль) — это не"""
    try:
        res = BigWorld.wg_collideSegment(spaceID, Math.Vector3(x, ref_y + band, z), Math.Vector3(x, ref_y - band, z), 128)
        while res is not None:
            hit_y = res[0].y
...
def _ground_drop_y(spaceID, x, z, ref_y):
    """Если узкая полоса пустая (танк в небе после подъёма) — луч вниз к земле."""
...
def _hull_hang_snap(spaceID, x, z, yaw, hw, hl, ref_y, band=2.2):
    """Опора как у WGVehicleFilter: 4 угла carryingTriangles."""
    hits = []
    for px, pz in _carrying_corners_world(x, z, yaw, hw, hl):
        gy = _carrying_ground_y(spaceID, px, pz, ref_y, band)
        if gy is not None:
            hits.append(gy)
    if hits:
...
def _terrain_speed_factor(descr, matKind):
    """Множитель хода (0-1] от сопротивления грунта под машиной ИМЕННО ЭТОГО"""
    try:
...
def _terrain_y_for_tilt(spaceID, x, z, ref_y, band=5.0):
    """Высота РЕЛЬЕФА под точкой (маска 18, как get_ground_height)."""
    try:
...
def _tilt_target_pitch_roll(spaceID, pos, yaw, descr):
    """Плоскость корпуса как у WGVehicleFilter: 4 угла carryingTriangles."""
    try:
    ...
    tr, tl, br, bl = _carrying_corners_world(x, z, yaw, hw, hl)
    y_tr = _terrain_y_for_tilt(spaceID, tr[0], tr[1], y)
    y_tl = _terrain_y_for_tilt(spaceID, tl[0], tl[1], y)
    y_br = _terrain_y_for_tilt(spaceID, br[0], br[1], y)
    y_bl = _terrain_y_for_tilt(spaceID, bl[0], bl[1], y)
    ...
    h_front = _avg(y_tr, y_tl, y)
    h_back = _avg(y_br, y_bl, y)
    h_right = _avg(y_tr, y_br, y)
    h_left = _avg(y_tl, y_bl, y)
...
def _carrying_sit_y(spaceID, x, z, yaw, pitch, roll, hw, hl, origin_y):
    """После pitch/roll точка опоры (локальный Y=0) не на средней высоте четырёх"""
    try:
        rm = Math.Matrix()
    ...
    for lx, lz in ((hw, hl), (-hw, hl), (hw, -hl), (-hw, -hl)):
        try:
            ...
            wx, wy, wz = x + v.x, origin_y + v.y, z + v.z
            gy = _terrain_y_for_tilt(spaceID, wx, wz, origin_y)
            if gy is not None:
                d = gy - wy
                if d > lift:
                    lift = d
...
def _slope_speed_target(h_front, h_back, half_L, direction):
    """Целевой (ещё НЕ сглаженный по времени - см."""
...
def _slope_ease_toward(current, target, dt):
    """Один шаг сглаживания множителя уклона к своей цели с ограниченной"""
    if current < target:
...
def _nav_probe_ground(spaceID, x, z):
    """(x,z) -> (groundY, normalY) поверхности под точкой, либо None,"""
    try:
...
def _nav_point_ok(spaceID, x, z, minPlaneY=0.75):
    """Годится ли (x,z) как точка маршрута: под ней настоящая земля (не"""
...
def _nav_step_blocked(dy, max_step):
    """Единая мера "это слишком большой скачок высоты, чтобы физически"""
    if dy < 0.0:
        return abs(dy) > max_step * 1.15
    return dy > max_step

def _nav_track_climb(ref, cur_x, cur_z, cur_y):
    """Отслеживает СУММАРНЫЙ набор высоты машиной относительно последней"""
    if ref is None:
...
def _nav_path_clear(spaceID, x0, z0, x1, z1, minPlaneY=0.75, samples=5):
    """Грубая (не полноценный pathfinding, но НАМНОГО лучше, чем ничего)"""
    try:
        _dx = x1 - x0
        _dz = z1 - z0
...
def _nav_sightline_dist(spaceID, x, z, dirX, dirZ, maxDist=300.0, height=2.2):
    """Дальность чистой видимости из точки (x,z) в направлении (dirX,dirZ)"""
...
def _xz_near_vehicle(x, z, ignore_id=None, rad=4.2):
    """Точка рядом с живым танком (XZ). Для навигации: чужой корпус — не здание."""
    try:
...
def _nav_hit_is_static_wall(res, ox, oz):
    """Горизонтальный луч упёрся в статику, а не в свой/чужой корпус."""
    if res is None:
        return False
    try:
        hit = res[0]
...
def _bot_los_blocked(spaceID, gunPos, aimPos, targetVeh):
    """True = между стволом и целью непробиваемая преграда (дом/скала/рельеф)."""
    try:
...
def _nav_scan_best_heading(spaceID, x, y, z, cur_yaw):
    """Веерное сканирование препятствий вокруг точки (x,y,z): пускает"""
...
def _feeler_ray_clear(spaceID, x, z, y, ang, height, hw, hl, dist):
    """Проверяет, свободна ли полоса шириной ~корпуса с боковыми запасами"""
    cs = _math.sin(ang)
    cc = _math.cos(ang)
    ox = _math.cos(ang)
    oz = -_math.sin(ang)
...
def _bot_avoid_scan(spaceID, x, y, z, cur_yaw, side):
    """Порт ADVANCED MULTI-RAY SENSORS из 0.8.2 (OffHangar, объезд"""
    _rays = [0.0]
...
def _nav_drivable_rise(spaceID, x, y, z, yaw, look, fw):
    """Порт DRIVABLE-SLOPE GUARD из 0.8.2 (_check_horizontal_collision):"""
    try:
        _seg_n = 6
...
def _tank_hull_dims(td):
    """Ширина/нос/корма по AABB уже загруженного collision BSP, иначе carryingPoint."""
    hw = 0.0
    hlf = 0.0
    hlb = 0.0
    got = False
...
def _bsp_load_parts(td, names=('chassis', 'hull', 'turret')):
    ...
    for name in names:
        try:
            ...
            if ht is not None and not ht.isBspModelLoaded():
                ht.loadBspModel()
...
def _mesh_samples_for(td):
    """Точки на ПОВЕРХНОСТИ collision BSP (лучи снаружи внутрь), не углы AABB."""
    name = getattr(td, 'name', None)
    if name in _G_MESH_SAMPLES:
        return _G_MESH_SAMPLES[name]
    samples = []
    if not _bsp_load_parts(td, ('chassis', 'hull')):
        if name is not None:
            _G_MESH_SAMPLES[name] = samples
        return samples
    seen = {}

    def _add_hit_points(ht, bb, to_veh):
        try:
            mn, mx = bb[0], bb[1]
        except Exception:
            return
        ...
        pad = 0.45
        starts = []
        for z in (mn[2] - pad, mx[2] + pad):
            for x in (mn[0], cx, mx[0]):
                starts.append(Math.Vector3(x, cy, z))
                starts.append(Math.Vector3(x, (cy + mx[1]) * 0.5, z))
        for x in (mn[0] - pad, mx[0] + pad):
            for z in (mn[2], cz, mx[2]):
                starts.append(Math.Vector3(x, cy, z))
        ...
        for start in starts:
            ...
            p = start + direc * dist
            vp = to_veh(p)
            seen[(round(vp.x, 2), round(vp.y, 2), round(vp.z, 2))] = (vp.x, vp.y, vp.z)

    try:
        ht = td.chassis['hitTester']
        _add_hit_points(ht, ht.bbox, lambda p: p)
    except Exception:
        pass
    try:
        ht = td.hull['hitTester']
        ...
        _add_hit_points(ht, ht.bbox, _hull_to_veh)
    except Exception:
        pass
...
def _world_point_hits_vehicle(vehicle, worldPt, x, y, z, yaw, pitch, roll, radius=0.10):
    ...
    if td is None:
        return False
    _bsp_load_parts(td, ('chassis', 'hull'))
...
def _tanks_mesh_overlap(vehA, ax, ay, az, ayaw, apitch, aroll,
    ...
    """True/False пересечение сеток. None — BSP ещё нет, оставляем OBB."""
    ...
    try:
        for sx, sy, sz in samplesA:
            wp = matA.applyPoint(Math.Vector3(sx, sy, sz))
            if _world_point_hits_vehicle(vehB, wp, bx, by, bz, byaw, bpitch, broll):
                return True
        for sx, sy, sz in samplesB:
            wp = matB.applyPoint(Math.Vector3(sx, sy, sz))
...
def _tank_circles(x, z, yaw, td):
    """Цепочка кругов вдоль корпуса (0.8.2:5584). Шаг r*1.5, r = max(hw,0.8)."""
    try:
...
def _tank_resolve(ax, az, ayaw, atd, bx, bz, byaw, btd, a_vx=0.0, a_vz=0.0, b_vx=0.0, b_vz=0.0, ay=None, by=None):
    """Детекция + ответ танк-танк (0.8.2:5607-5662). Broadphase 12м + height gate 3м, narrow - circle-vs-circle."""
    try:
...
def _ram_damage(closing_speed, mass_self, mass_other):
    """Урон тарана 0.8.2 physics.py:592."""
...
def _check_horizontal_collision(spaceID, pos, yaw, td):
    """Стена: 3 колонки × 3 луча + drivable guard. Возвращает (x, z, hit_res) или None."""
    try:
        hw, hlf, hlb = _tank_hull_dims(td)
        try:
            look = hlf + hlb
...
def _fell_destructibles_near(spaceID, px, pz, yaw, descr, cooldown=None, chunk_counts=None):
    """Валка TREE/FALLING_ATOM/FRAGILE по AreaDestructibles 0.6.2.8:"""
    try:
        ...
        if chunk_counts is None:
            chunk_counts = _scenery_counts
        try:
            hw, hlf, hlb = _tank_hull_dims(descr)
            reach = max(float(hw) + 1.2, 2.4)
        except Exception:
            reach = 3.2
        _dNow = BigWorld.time()
        try:
            _cx, _cz = chunkIndexesFromChunkID(chunkIDFromPosition(Math.Vector3(px, 0.0, pz)))
        except Exception:
            return
        for _gx in (_cx - 1, _cx, _cx + 1):
            for _gz in (_cz - 1, _cz, _cz + 1):
                _chunkID = chunkIDFromChunkIndexes(_gx, _gz)
                if not g_destructiblesManager.isChunkLoaded(_chunkID):
                    continue
                _dCount = int(chunk_counts.get(_chunkID, 0) or 0)
                if _dCount <= 0:
                    continue
                _chunkMat = BigWorld.wg_getChunkMatrix(spaceID, _chunkID)
                if _chunkMat is None:
                    continue
                _origin = _chunkMat.translation
                for _di in xrange(_dCount):
                    _dKey = (_chunkID, _di)
                    if _dKey in _G_DESTR_BROKEN:
                        continue
                    if _dNow - cooldown.get(_dKey, 0.0) < 0.4:
                        continue
                    if _is_protected_cap_scenery(spaceID, _chunkID, _di):
                        continue
                    _destrMat = BigWorld.wg_getDestructibleMatrix(spaceID, _chunkID, _di)
                    if _destrMat is None:
                        continue
                    _dWorldPos = _origin + _destrMat.translation
                    _ddx = px - _dWorldPos.x
                    _ddz = pz - _dWorldPos.z
                    if (_ddx * _ddx + _ddz * _ddz) >= (reach * reach):
                        continue
                    _dDesc = _destr_desc(spaceID, _chunkID, _di)
                    if _dDesc is None:
                        continue
                    _dType = _dDesc['type']
                    if _dType == DESTR_TYPE_STRUCTURE:
                        continue
                    if _dType in (DESTR_TYPE_TREE, DESTR_TYPE_FALLING_ATOM):
                        _fallYaw = yaw if yaw is not None else _math.atan2(-_ddx, -_ddz)
                        _fallData = encodeFallenDestructible(_di, encodeFallenParams(_fallYaw, 1))
                        g_destructiblesManager.orderDestructibleDestroy(_chunkID, 0, _fallData, True)
                        cooldown[_dKey] = _dNow
                        _G_DESTR_BROKEN.add(_dKey)
                    elif _dType == DESTR_TYPE_FRAGILE:
                        g_destructiblesManager.orderDestructibleDestroy(_chunkID, 1, _di, True)
                        cooldown[_dKey] = _dNow
                        _G_DESTR_BROKEN.add(_dKey)
...
def _face_yaw(fromX, fromZ, toX, toZ):
    """Рыскание (рад) корпуса, чтобы машина из (fromX, fromZ) смотрела на"""
    _dx = toX - fromX
    _dz = toZ - fromZ
...
def _arena_center_xz(arena):
    """Центр карты из arena.typeDescriptor.boundingBox — то же поле, что"""
    try:
...
def _fmod_sound_play(snd):
    """Как MusicController.play / Scaleform/utils/sound.py: объект FMOD, метод play()."""
    if snd is None:
        return False
    try:
        snd.play()
...
def _offline_prepare_music(mc=None):
    """Грузит группы music/ambient из FMOD и перечитывает gui/music_events.xml."""
    import FMOD
...
def _coords_centroid(coords):
    """Центр списка точек спавна [(x, y, z), ...] -> (средний x, средний z)."""
...
def _nav_clear_forward(spaceID, x, z, dirX, dirZ, height, maxDist):
    """Дальность чистой видимости из (x,z) в направлении (dirX,dirZ) на"""
    try:
...
def _nav_elevation(spaceID, x, z, radius=25.0):
    """Насколько (x,z) выше среднего уровня местности вокруг (4 точки на"""
    try:
        gp = _nav_probe_ground(spaceID, x, z)
        if gp is None:
            return 0.0
...
def _solve_ballistic_dir(gunPos, target, speed, gravity, lofted, maxElevation=None):
    """БАГФИКС АРТ-ПРИЦЕЛА: раньше направление выстрела для арт-режима (и для"""
...
def _solve_arty_shot(gunPos, target, speed, gravity, maxElevation):
    """БАГФИКС "АРТА ПОПАДАЕТ": _solve_ballistic_dir с maxElevation здесь"""
    dx = target.x - gunPos.x
    dz = target.z - gunPos.z
    horiz = _math.sqrt(dx * dx + dz * dz)
    if horiz < 0.05:
...
def _player_cruise_available(avatar):
    """КРУИЗ-КОНТРОЛЬ ДОСТУПЕН: в ваниле автопилот нельзя включить, пока не"""
    try:
...
def _player_cruise_toggle(avatar):
    """КРУИЗ-КОНТРОЛЬ игрока: тумблер вкл/выкл (по умолчанию клавиша R, см."""
...
def _player_cruise_step(avatar, delta):
    """КРУИЗ-КОНТРОЛЬ игрока: клавиши +/- (CMD_INCREMENT/DECREMENT_CRUISE_MODE)"""
    if not _player_cruise_available(avatar): return
...
def _player_cruise_cancel(avatar):
    """КРУИЗ-КОНТРОЛЬ игрока: снятие круиза по нажатию W/S (как в ваниле - любое"""
    try:
        if getattr(avatar, '_cruise_active', False):
            avatar._cruise_active = False
            avatar._cruise_target = 0.0
...
def _killfeed_display_name(battle, veh_id):
    """КИЛЛФИД: имя танка в ленте убийств."""
    try:
...
def _killfeed_show(battle, key, args):
    """КИЛЛФИД: показать сообщение НАТИВНОЙ панелью 0.4.5 (FadingMessagesPanel"""
    try:
...
def _show_dead_damage_panel():
    """ДОХЛЫЙ ТАНК = ДОЛЖЕН БЫТЬ РАЗБИТ ЦЕЛИКОМ: приводит дамаг-панель 0.4.5"""
    try:
        from gui.WindowsManager import g_windowsManager
        _bw = getattr(g_windowsManager, 'battleWindow', None)
        if _bw is None:
...
def _offline_timer_setArenaTime(self_win):
    """Один тик таймера арены (подменяет ванильный Battle.__setArenaTime)."""
...
def _ballistic_flight_time(launchPos, initVelocity, gravityMag, targetPos):
    """Время полёта снаряда до точки попадания."""
...
def _trace_ballistic_hit(spaceID, shotPos, initVelocity, gravityMag, maxDist, exceptIDs):
    """БАГФИКС ПОПАДАНИЙ АРТЫ: раньше проверка попадания у игрока (ниже, в"""
...
def _vehicle_world_matrix(vehicle):
    """Авторитетная МИРОВАЯ матрица оффлайн-танка для коллизий."""
...
def _vehicle_gun_world_pos(veh, fallback_height=1.8):
    """Точка HP_gunFire как в Avatar.shoot, иначе высота корпуса+башни."""
    try:
...
def _entity_world_pos(veh):
    """Мировая точка танка: та же матрица, что хиттест (_vehicle_world_matrix),"""
    if veh is None:
...
def _pt_xyz(pt):
    """Точка взрыва как (x,y,z): Vector3 или кортеж collideSegment."""
    if pt is None:
        return None
    try:
...
def _shell_he_radius(shell):
    """Радиус ОФ как в vehicles.py (explosionRadius, иначе caliber²/5555)."""
    r = 0.0
    cal = 0.0
    try:
...
def _stamp_entity_pose(veh, matrix=None):
    """Пишем движковый position с живой позы, иначе маркеры/автоприцел/взрыв на спавне."""
    if veh is None:
        return
    if matrix is None:
...
def _hp_gui_height(veh):
    """Высота точки HP_gui по bbox корпуса+башни, если ноды модели нет."""
    off_y = 2.35
    try:
...
def _hp_gui_provider(veh):
    """Как Vehicle.startVisual: mProv = self.model.node('HP_gui')."""
    if veh is None:
        return None
    try:
...
def _sync_gui_matrix(veh, matrix=None):
    """Запасная мировая матрица маркера, если ноды HP_gui ещё нет."""
    if veh is None:
        return None
    if matrix is None:
        matrix = getattr(veh, '_offline_matrix', None)
...
def _camera_shake(duration=0.35, strength=0.14):
    """Как ShockWave при взрыве СВОЕГО танка: CursorCamera.shake + короткий кик pivot."""
    cam = None
    try:
        cam = BigWorld.camera()
    except Exception:
...
def _apply_hit_impulse(victim, shot_from, penetrated=True):
    """Попадание по игроку: трясём ЭКРАН (как ShockWave при гибели), не корпус."""
    try:
        p = BigWorld.player()
...
def _vehicle_collide_segment_component(vehicle, startPoint, endPoint):
    """То же самое, что оригинальный Vehicle.collideSegment() (см."""
    descr = vehicle.typeDescriptor
    try:
        for _htName in ('chassis', 'hull', 'turret', 'gun'):
            _ht = getattr(descr, _htName, None)
            if _ht is None:
                continue
            _tester = _ht.get('hitTester') if isinstance(_ht, dict) else None
            if _tester is not None and not _tester.isBspModelLoaded():
                try:
                    _tester.loadBspModel()
                except Exception:
                    pass
        if descr.hitTester is not None and not descr.hitTester.isBspModelLoaded():
            descr.hitTester.loadBspModel()
    except Exception:
        pass
    ...
    try:
        if descr.hitTester is not None:
            if not descr.hitTester.isBspModelLoaded():
                descr.hitTester.loadBspModel()
    except Exception:
        pass
...
def _vehicle_components_with_names(vehicle):
    """Копия Vehicle.getComponents() (оригинал), дополненная именем"""
...
def _collide_dynamic_component(startPoint, endPoint, exceptIDs):
    """Аналог ProjectileMover.collideDynamic() (оригинал) - перебирает все"""
    res = None
    direction = endPoint - startPoint
    direction.normalise()
    for vehicleID in list(BigWorld.player().arena.vehicles.iterkeys()):
        ...
        if collRes is None:
            try:
                vanilla = vehicle.collideSegment(startPoint, endPoint)
                if vanilla is not None:
                    collRes = (vanilla[0], vanilla[1], vanilla[2], None)
            except Exception:
                collRes = None
        if collRes is None:
            continue
        ...
        if dist < startPoint.distTo(endPoint):
            endPoint = startPoint + direction * dist
            res = (vehicle, dist, collRes[1], collRes[2], collRes[3])

    return res

def _veh_velocity_2d(veh):
    """Вектор скорости танка (м/с) в мировых координатах по его текущему курсу"""
    try:
        _ob = None
        try:
...
def _offline_play_collision_fx(veh, hitPt):
    """Ваниль Vehicle.showVehicleCollisionEffect (effects/collision в XML танка)."""
    try:
...
def _predict_aim_point(gunPos, aimPoint, speed, gravity, targetVeh, maxIter=4):
    """УПРЕЖДЕНИЕ ПО ДВИЖУЩЕЙСЯ ЦЕЛИ - точка прицеливания в позицию, где цель"""
    _vel = None
    try:
...
def _lead_ballistic_dir_for_gun(gunPos, aimPoint, speed, gravity, exceptIDs):
    """Упреждающее НАПРАВЛЕНИЕ для прямого настильного выстрела игрока: если в"""
    try:
...
def _ballistic_arrival_pos(launchPos, initVelocity, gravityMag, t):
    """Положение снаряда на его параболе в момент времени t - аналитическое"""
    try:
...
def _clamp_impact_to_veh(veh, arrivalPt, maxProbe=18.0):
    """МОМЕНТ ПРИЛЁТА vs МОМЕНТ ВЫСТРЕЛА: пока снаряд летел, танк мог уехать,"""
...
def _pick_damage_sticker_id(isPen, isRicochet=False):
    """Выбор ID текстуры декали попадания из кэша damageStickers (порт из 0.8.2"""
    try:
...
def _comp_local_segment(veh, compName, worldStart, worldEnd):
    """Переводит мировые концы отрезка в ЛОКАЛЬНЫЕ координаты компонента танка"""
    _fallback = None
    try:
...
def _add_hit_decal(veh, compName, worldHitPos, worldDir, isPen, isRicochet=False, jitter=0.0):
    """ДЕКАЛИ ПОПАДАНИЙ (порт из 0.8.2): постоянная отметина ровно в том месте,"""
    try:
...
def _calc_shot_penetration(shotData, targetDescr, targetYaw, shotDirWorld):
    """Расчёт пробития снаряда бота по той же формуле, что используется для"""
    try:
...
# STREAMING_CHUNK:Setting up dummy classes and filters...
...
# STREAMING_CHUNK:Initializing the Avatar class (Player logic)...
def _apply_offline_crew_and_eq_bonuses(descr, veh_inv_id):
    """Перки экипажа + уже установленное оборудование (miscAttrs устройств)."""
    if descr is None:
        return
    try:
...
class SimpleAvatar:
    ...
    def handleVehicleCollidedVehicle(self, vehA, vehB, hitPt, time):
        """Клиент 0.4.5: Avatar.handleVehicleCollidedVehicle — искра столкновения."""
        try:
    ...
    def broadcast(self, cid, message):
        """Вызывается штатной flash-панелью чата боя (MessengerBattleInterface."""
        try:
            if not message:
                return
...
# STREAMING_CHUNK:Implementing shooting logic for Player...
    def shoot(self):
        veh = self.getVehicleAttached()
        if not veh: return
    ...
    def _apply_vehicle_damage(self, veh, dmg_val, origShotPos, shell_type=ShellType.AP, penetrated=True, hit_component=None, killer_id=None, fire_damage=False):
        """Единая функция нанесения урона + всех сопутствующих обновлений (хп, смерть,"""
        try:
    ...
    def lockOn(self, target):
        """Автоприцел 0.4.5 (control_modes -> player.lockOn / Avatar.lockOn)."""
    ...
    def onLockedVehicleLost(self):
        """Avatar.onLockedVehicleLost: цель умерла или ушла из засвета."""
            ...
            if _ob is not None:
                try:
                    if artefact.tags & frozenset(('repairkit',)):
                        _names = [deviceName]
                        if deviceName == 'chassis':
                            _names = ['leftTrack', 'rightTrack']
                        _ob._repairCriticalIcon(deviceName)
                        try:
                            _st = _ob._get_dmg_vehicle_state(self.getVehicleAttached())
                            for _dn in _names:
                                _mod = _st.get_module(_dn) if _st is not None else None
                                if _mod is not None and hasattr(_mod, 'repair_full'):
                                    _mod.repair_full()
                                ...
                                try:
                                    ...
                                    if bw and hasattr(bw, 'damagePanel'):
                                        bw.damagePanel.updateCriticalIcon(_dn, 'normal')
                                except Exception:
                                    pass
                            try:
                                bw = _ob.battleWindow
    ...
    def showNoPenArc(self, gYaw):
        """Чёрная дуга непробития: тот же смысл, что Aim.showHit (DamageIndicator.swf),"""
        try:
            import GUI
        except Exception:
            return
...
def make_vehicle_state(name, max_hp, descr=None):
    """Собирает объект DmgVehicle (модули + экипаж) для DamageResolver -"""
...
# STREAMING_CHUNK:Setting up OfflineBattle Core...
class OfflineBattle:
...
# STREAMING_CHUNK:Implementing visual setup and overrides...
                ...
                try:
                    ...
                    if wreck:
                        ...
                        if _pm_target is None:
                            _pm_target = getattr(wreck, '_offline_matrix', wreck.matrix)
...
# STREAMING_CHUNK:Running the battle loop and ticking logic...
    ...
    def _botChatTick(self):
        """Раз в несколько секунд случайный живой бот кидает случайную фразу"""
        if getattr(self, '_is_finished', False): return
        try:
    ...
    def _get_dmg_vehicle_state(self, veh):
        """Возвращает объект DmgVehicle (модули + экипаж) для данного танка,"""
        try:
            vehID = veh.id
        except Exception:
            return None
    ...
    def _destroy_player_modules_and_crew(self, veh):
        """Гибель танка игрока: ещё целые модули становятся уничтоженными,"""
        try:
            if veh is None: return
    ...
    def _apply_ramming(self, veh_a, veh_b, nx, nz, hit_pt, mass_a, mass_b, vn):
        """Таран как в клиенте 0.4.5: искра collision + урон HP обеим + крит гусеницы"""
        try:
    ...
    def on_penetrating_hit(self, veh, damage, shell_type=ShellType.AP, penetrated=True, hit_component=None):
        """Точка входа в подсистему модулей/экипажа: вызывается сразу после"""
        state = self._get_dmg_vehicle_state(veh)
    ...
    def _sync_crashed_track_visual(self, veh, module_type, show):
        """Включить/выключить ВИДИМУЮ "отвалившуюся" гусеницу для танка veh."""
        try:
            appr = getattr(veh, 'appearance', None)
            if appr is None:
                return
    ...
    def _post_damage_notice(self, text, colour):
        """ЭКРАННОЕ ПРЕДУПРЕЖДЕНИЕ О ПОВРЕЖДЕНИИ: текст повреждений"""
        try:
            import GUI
        except Exception:
            return
    ...
    def _attacker_name(self, vehID):
        """Имя атакующего машину vehID для полной фразы из 0.8.2."""
        try:
    ...
    def _post_damage_message(self, module_type, mstate, vehID=None):
        """ПРЕДУПРЕЖДЕНИЕ О ПОВРЕЖДЕНИИ МОДУЛЯ: полная фраза 0.8.2"""
    ...
    def _post_crew_message(self, role, outcome, vehID=None):
        """То же для экипажа: "Выстрел <имя> вывел из строя <член>." оранжевым."""
    ...
    def _post_death_message(self, attacker_id=None):
        """Гибель танка игрока: "Танк уничтожен выстрелом <имя>." (redpurple),"""
    ...
    def _get_mobility_factor(self, vehID):
        """Множитель скорости/поворота (0.0-1.0) по состоянию гусениц,"""
        state = self._dmg_vehicle_states.get(vehID)
    ...
    def _can_fire(self, vehID):
        """False, только если орудие уничтожено ('destroyed', красное) - выстрел"""
        state = self._dmg_vehicle_states.get(vehID)
    ...
    def _reload_multiplier(self, vehID):
        """Во сколько раз дольше перезаряжается орудие из-за повреждённой"""
        state = self._dmg_vehicle_states.get(vehID)
        f = 1.0
        if state is not None:
    ...
    def _turret_speed_factor(self, vehID):
        """Множитель скорости поворота башни от повреждённого привода"""
        state = self._dmg_vehicle_states.get(vehID)
        if state is None: return 1.0
        return module_stat_factor(state, 'turret_speed')

    def _maybe_ignite_fire(self, veh, state, mstate):
        """Пробитый топливный бак может поджечь танк - настоящая механика"""
    ...
    def _stop_fire(self, veh):
        """Огнетушитель: Avatar.__showDamageIconAndPlaySound FIRE_STOPPED."""
        try:
            state = self._get_dmg_vehicle_state(veh)
            if state is None:
                return
    ...
    def _schedule_dmg_repair(self, vehID, module_type):
        """Завести таймер авто-ремонта одного сбитого модуля (красный ->"""
    ...
    def _drain_bot_repair_queue(self, vehID):
        """Очередь починки бота: выбрать СЛЕДУЮЩИЙ сбитый модуль после того, как"""
        if getattr(self, '_is_finished', False): return
        state = self._dmg_vehicle_states.get(vehID)
        if state is None: return
    ...
    def _repair_dmg_module(self, vehID, module_type):
        """Авто-ремонт - ПОРТ ИЗ 0.8.2: поднимает модуль из 'destroyed'"""
        if getattr(self, '_is_finished', False): return
        state = self._dmg_vehicle_states.get(vehID)
        if state is None: return
        module = state.get_module(module_type)
    ...
    def _repairCriticalIcon(self, moduleType):
        """'Чинит' значок модуля обратно в ЗЕЛЁНЫЙ - это путь ПОЛНОЙ починки"""
        if getattr(self, '_is_finished', False): return
        try:
    ...
    def _setupAmmoPanel(self):
        ...
        try:
            ...
            LOG_NOTE('[BATTLE] ammo panel: %s' % (av._ammo,))
        except Exception:
            LOG_CURRENT_EXCEPTION()

    def _vehicle_try_break_destructible(self, wallRes, headingYaw):
        """Общая проверка "во что физически упёрлась машина" - и игрок, и"""
        if wallRes is None:
            return None
        matKind = wallRes[2]
        if matKind < _const.DESTRUCTIBLE_MATKINDS_MIN or matKind > _const.DESTRUCTIBLE_MATKINDS_MAX:
    ...
    def _building_fields_for_chunk(self, spaceID, cid):
        """КОЛЛИЗИЯ ДОМОВ 2 - 'ровное поле по размерам дома' (по чанку)."""
    ...
    def _clamp_off_building_fields(self, spaceID, prev_x, prev_z, try_x, try_z, radius):
        """КОЛЛИЗИЯ ДОМОВ 2 - главная: точка ЦЕНТРА корпуса не может войти в"""
        try:
            if not hasattr(self, '_bld_fields_cache'):
                self._bld_fields_cache = {}
            _touched = False
    ...
    def _compute_accel_decel(self, descr):
        """Разгон/торможение/накат (м/с^2) и угловое ускорение поворота"""
        try:
    ...
    def _try_attach_player_motor(self, veh):
        """Пытается подменить мотор 3D-модели машины ИГРОКА на self._offline_servo -"""
    ...
    def _update_cruise_indicator(self):
        """КРУИЗ НА ДАМАГ-ПАНЕЛИ (СТРЕЛКА): вместо самодельного GUI.Text"""
    ...
    def __movementTick(self):
                ...
                for _off in (0.0, _pHw * 0.9, -_pHw * 0.9, _pHw * 0.45, -_pHw * 0.45):
                    ...
                    if _fwdRes is not None:
                        ...
                        if _destr_whisker_blocks(self.spaceID, _fwdRes[2], _fwdRes[4], _fwdRes[5]):
                            _rayS = _math.sin(_ray_yaw)
                            _rayC = _math.cos(_ray_yaw)
                            _hitD = max(0.0, min(_contact_len,
                                (_fwdRes[0].x - _startX) * _rayS + (_fwdRes[0].z - _startZ) * _rayC))
                            self._cur_pos.x = _prev_x + _rayS * _hitD
                            self._cur_pos.z = _prev_z + _rayC * _hitD
                            _blocked_by_wall = True
                            _contact_clamped = True
                            break

            if _blocked_by_wall:
                if not _contact_clamped:
                    self._cur_pos.x = _prev_x; self._cur_pos.z = _prev_z
                if not _p_hill_now:
                    self._cur_speed = 0.0
                    try:
                        _fxpt = None
        ...
        def _obb_mtv(ax, az, ayaw, ahw, ahl, bx, bz, byaw, bhw, bhl):
            """Возвращает (nx, nz, overlap) - единичный вектор выталкивания"""
        ...
        if not player_is_dead:
            _hang_snap_y, _hang_block = _hull_hang_snap(self.spaceID, self._cur_pos.x, self._cur_pos.z, self._cur_yaw, half_W, half_L, self._cur_pos.y)
        ...
        for _bot_vehID, _bot_descr, _bot_is_player in self.vehicles:
            ...
            try:
                _kst = getattr(self, '_bot_state', {}).get(_bot_vehID) or {}
                self._destr_keep_bushes = (_kst.get('position_role') == 'bush')
            except Exception:
            ...
            try:
                ...
                if _bot_mat is not None:
                    ...
                    if hasattr(_bot_veh, '_ai_desired_yaw'):
                        ...
                        try:
                            ...
                            if _pp is not None:
                                ...
                                if (_dxp * _dxp + _dzp * _dzp) > _BOT_MOVE_LOD_DIST2:
                                    if ((getattr(self, '_tdiag', 0) + int(_bot_vehID)) & 1):
                                        continue
                        except Exception:
                            pass
                        _new_yaw = _cur_yaw
                        ...
                        if _is_ai_reversing:
                            ...
                            if BigWorld.time() > _bot_veh._ai_rev_scan_time:
                                ...
                                if _rv_disp >= _BOT_REVERSE_FUTILE_DIST:
                                    ...
                                    _bot_veh._ai_reverse_until = 0.0
                                    _bot_veh._ai_reverse_blocked_until = BigWorld.time() + _BOT_REVERSE_FUTILE_COOLDOWN
                                    _bot_veh._ai_rev_target = None
                                    ...
                                    LOG_NOTE("[BATTLE][NAV] bot %d reverse making no progress - cancelled, dropping waypoint" % _bot_vehID)
                                else:
                            ...
                            if _rv_tgt is not None:
                                ...
                                if BigWorld.time() - _rv_start <= _BOT_REVERSE_TRAPPED_MAX:
                                    _bot_veh._ai_reverse_until = BigWorld.time() + _BOT_STUCK_REVERSE_DURATION
                                else:
                                    _revs_bot_state = getattr(self, '_bot_state', None)
                                    _revs_st = _revs_bot_state.get(_bot_vehID) if _revs_bot_state is not None else None
                                    if _revs_st is not None:
                                        ...
                                        _revs_st['bad_waypoint_time'] = BigWorld.time()
                                        _revs_st['wp_timer'] = 0.0
                                    _lg_t = getattr(_bot_veh, '_ai_trap_log_t', 0.0)
                                    if BigWorld.time() - _lg_t >= 8.0:
                                        _bot_veh._ai_trap_log_t = BigWorld.time()
                                        LOG_NOTE("[BATTLE][NAV] bot %d trapped reverse budget spent (%.0fs no escape) - dropping waypoint" % (
                                ...
                                if _sw_h >= 5.0 or (_sw_t - _swt) >= _BOT_SLOPE_STUCK_WINDOW:
                                    ...
                                    if _slope_stuck_now:
                                        ...
                                        if _prog_st is not None:
                                            ...
                                            LOG_NOTE("[BATTLE][NAV] bot %d slope-stuck (rise=%.1fm horiz=%.1fm dir=%.0f deg) - reversing, ridge dir stored" % (
                                                _bot_vehID, _sw_rise, _sw_h, _math.degrees(_slope_ang)))
                                        _bot_veh._ai_reverse_until = _sw_t + _BOT_STUCK_REVERSE_DURATION
                                        _bot_veh._ai_reverse_side = _random.choice([-1, 1])
                        ...
                        if abs(_spd) > 0.5 and not _bot_hill_now:
                            ...
                            if _step_dist > 1.2:
                                ...
                                for _si in range(1, _n_samples + 1):
                                    ...
                                    if _segRes is not None:
                                        _destr_seg = self._vehicle_try_break_destructible(_segRes, _new_yaw)
                                        if _destr_seg == 'soft':
                                            pass
                                        else:
                                    ...
                                    if _segFwd is not None and not _seg_hit_is_ground(_segFwd, 0.40, _cur_pos.y):
                                        self._vehicle_try_break_destructible(_segFwd, _new_yaw)
                        ...
                        if not _blocked and _impact_kind is None and not _bot_hill_now:
                            try:
                                ...
                                _destr_kind = self._vehicle_try_break_destructible(_wallRes, _new_yaw)
                                if _destr_kind == 'soft':
                                    pass
                                    ...
                                    try:
                                        ...
                                        for _b_side in (-1.0, 1.0):
                                            ...
                                            if _bSideRes is None:
                                                continue
                                            _b_side_destr = self._vehicle_try_break_destructible(_bSideRes, _new_yaw)
                        ...
                        if not _blocked:
                            ...
                            for _ovID, _, _ in self.vehicles:
                                ...
                                _bmesh = None
                                if (_mtv_now is not None) or (_dx_now * _dx_now + _dz_now * _dz_now < 64.0):
                                    try:
                                ...
                                elif _mtv_now is not None:
                                    _mtv_now = (_mtv_now[0], _mtv_now[1], max(_mtv_now[2], 0.25))
                        ...
                        if _blocked:
                            ...
                            if not (_is_tank_block and _tank_waiter):
                                ...
                                if (_avoid_yaw_diff > 2.0 or _avoid_yaw_diff < -2.0) and not _rb_is_reversing \
                                        and BigWorld.time() >= getattr(_bot_veh, '_ai_reverse_blocked_until', 0.0):
                                ...
                                if not hasattr(_bot_veh, '_ai_stuck_since'):
                                    _bot_veh._ai_stuck_since = BigWorld.time()
                                elif BigWorld.time() - _bot_veh._ai_stuck_since > _BOT_REVERSE_AFTER_BLOCKED \
                                        and not _rb_is_reversing \
                                        and BigWorld.time() >= getattr(_bot_veh, '_ai_reverse_blocked_until', 0.0):
                    ...
                    _b_hang_y, _b_hang_block = _hull_hang_snap(self.spaceID, _bp.x, _bp.z, _byaw, _bhW, _bhL, _bp.y)
                    ...
                    try:
                        _scenery_run_over(_bot_veh, (_sfx, _sfz), (_bp.x, _bp.z), dt)
                    except Exception:
                        pass
    ...
    def _get_map_bounds(self):
        """Реальные границы текущей карты (x0,z0,x1,z1) с отступом от края"""
    ...
    def _start_nav_grid_build(self):
        """БОЛЬШОЙ БАГФИКС НАВИГАЦИИ - "СЕТКА ПРОХОДИМОСТИ" КАРТЫ:"""
    ...
    def _nav_grid_build_step(self):
        ...
        try:
            ...
            while q and n < _NAV_GRID_BATCH:
                ...
                n += 1
            if q:
                BigWorld.callback(0.03, self._nav_grid_build_step)
                return
    ...
    def _maybe_retry_sparse_grid(self):
        """См. комментарий у _NAV_GRID_SPARSE_FLOOR выше по файлу. Если сразу"""
    ...
    def _nav_grid_flood_fill(self):
        """Заливка от точек респауна обеих команд по 8-связным соседям"""
        seeds = []
        arena_id = getattr(self, '_current_arena_id', None)
    ...
    def _nav_grid_candidates(self, anchor, spread, avoid=None):
        """Подтверждённо достижимые клетки сетки в радиусе spread от"""
    ...
    def _nav_pick_waypoint(self, cls, anchor, my_pos, spread, avoid=None, seek_dir=None, avoid_dir=None):
        """Подбирает ПРОВЕРЕННУЮ через коллизию движка точку маршрута рядом"""
        ...
        if fallback_pt is not None:
            try:
                _fdx = fallback_pt.x - my_pos.x
                _fdz = fallback_pt.z - my_pos.z
                if (_fdx * _fdx + _fdz * _fdz) < 14400.0:
                    return fallback_pt
            except Exception:
                return fallback_pt
            return Math.Vector3(my_pos.x, my_pos.y, my_pos.z)
        LOG_NOTE("[BATTLE][NAV] No drivable candidate found near (%.0f, %.0f), holding position" % (anchor.x, anchor.z))
        return Math.Vector3(my_pos.x, my_pos.y, my_pos.z)
    ...
    def _get_lane_geometry(self, team):
        """Направление "вперёд" от своей базы к вражеской (единичный вектор fwd)"""
    ...
    def _resolve_curated_point(self, team, depth_frac, lane_frac):
        """Переводит заготовленную точку маршрута (depth_frac вдоль линии база-база,"""
        zones = self._get_base_zones()
        my_base_pos = Math.Vector3(0, 0, 0)
        enemy_base_pos = Math.Vector3(0, 0, 0)
        for b_team, zone in zones.items():
    ...
    def _get_patrol_zone_defs(self, arena_id, team):
        """Список зон патруля для текущей карты (см."""
    ...
    def _patrol_build_order(self, zl):
        """Порядок обхода зон для ТУРА по всей карте: индексы, отсортированные"""
        try:
            _idx = range(len(zl))
    ...
    def _patrol_advance(self, st, zl):
        """Следующая зона тура (пинг-понг по глубине, без дальнего прыжка"""
        try:
    ...
    def _route_pick(self, st, cls, team, my_real_pos, avoid=None, avoid_dir=None):
        """Заготовленный маршрут ROUTES как тур по всей карте (приоритет над"""
        try:
    ...
    def _route_advance_on_arrive(self, st, cls):
        """Следующий узел ROUTES-цепочки (пинг-понг, без прыжка через карту)."""
        try:
            _variants = ROUTES.get(getattr(self, '_current_arena_id', None), {}).get(cls, [])
        except Exception:
            _variants = []
    ...
    def _patrol_zone_pick(self, st, cls, team, my_real_pos, avoid=None, avoid_dir=None):
        """ТУР ПО ЗОНАМ (фолбэк, если для класса нет ROUTES-цепочки): точка"""
...
# STREAMING_CHUNK:Bot Logic (AITick) with new duals system and tracer LoS...
    def _botAITick(self):
        if getattr(self, '_is_finished', False): return
        ...
        try:
            now = BigWorld.time()
            if not hasattr(self, '_bot_state'): self._bot_state = {}
            ...
            for vID, vDescr, vIsPlayer, vTeam, vVehObj in alive:
                ...
                for eID, eDescr, eIsPlayer, eTeam, eVehObj in alive:
                    ...
                    if (v_pos - _real_pos(eVehObj)).length <= v_radius:
                        _los_clear_spot = True
                        try:
                            _spot_obs = Math.Vector3(v_pos.x, v_pos.y + 2.0, v_pos.z)
                            _spot_tgt = Math.Vector3(_real_pos(eVehObj))
                            _spot_tgt.y += 1.0
                            if _bot_los_blocked(self.spaceID, _spot_obs, _spot_tgt, eVehObj):
                                _los_clear_spot = False
                        except Exception:
                            _los_clear_spot = False
                        if _los_clear_spot:
            ...
            for vehID, descr, isPlayer, team, veh in alive:
                ...
                if 'class' not in st:
                    ...
                    if v_class == 'SPG':
                        pos_role = 'base'
                    elif v_class == 'TD' and _random.random() < 0.70:
                        pos_role = 'bush'
                ...
                if best is None or (best[3] > 450.0 and st['class'] != 'SPG' and not best_is_provoked):
                    ...
                    if now > st.get('wp_timer', 0.0):
                        ...
                        if role in ('base', 'bush') and cls != 'SPG':
                        ...
                        if _capturing_base:
                            ...
                            spawn_dir = Math.Vector3(st['spawn_pos'].x, 0, st['spawn_pos'].z)
                            if spawn_dir.length > 0.1: spawn_dir.normalise()
                            else: spawn_dir = Math.Vector3(0, 0, 1)

                            _anchor = Math.Vector3(st['spawn_pos'].x + spawn_dir.x * _random.uniform(20, 50),
                                                    0, st['spawn_pos'].z + spawn_dir.z * _random.uniform(20, 50))
                            st['waypoint'] = self._nav_pick_waypoint('SPG', _anchor, my_real_pos, 20.0,
                                                                      avoid=_avoid_pt, avoid_dir=_avoid_dir)
                            st['wp_timer'] = now + 15.0
                            ...
                            if st.get('bush_point') is None:
                                ...
                                if bdir.length > 0.1: bdir.normalise()
                                else: bdir = Math.Vector3(0, 0, 1)
                                bdist = _random.uniform(40.0, 90.0)
                                _anchor = Math.Vector3(st['spawn_pos'].x + bdir.x * bdist,
                                                        0, st['spawn_pos'].z + bdir.z * bdist)
                                st['bush_point'] = self._nav_pick_waypoint(cls, _anchor, my_real_pos, 20.0, avoid_dir=_avoid_dir)
                            st['waypoint'] = st['bush_point']
                            st['wp_timer'] = now + 60.0
                    ...
                    if st.get('position_role') == 'push' \
                            and (st.get('patrol_zone_idx') is not None or st.get('route_idx') is not None) \
                            and st.get('_capture_pt') is None \
                            and diff_wp.length <= _PATROL_ARRIVE_DIST:
                                        ...
                                        if getattr(self.playerAvatar, 'inputHandler', None):
                                            try:
                                                ...
                                                if pm_ctrl:
                                                    ...
                                                    if _pm_target is None:
                                                        _pm_target = getattr(_tVeh, '_offline_matrix', _tVeh.matrix)
                                                    pm_cam_bw.target = _pm_target
                                            except Exception:
...
# STREAMING_CHUNK:Arena update functions and resource cleanup...
    def _updateCapturePoints(self):
        ...
        for base_team, zone in zones.items():
            ...
            for vehID, descr, isPlayer in self.vehicles:
                veh = BigWorld.entity(vehID)
                if veh and getattr(veh, 'health', 0) > 0:
                    pos = getattr(veh, '_offline_matrix', veh.matrix).translation if hasattr(veh, '_offline_matrix') else veh.position
                    ...
                    if horiz_dist <= zone['radius']:
                        ...
                        if veh_team == attacking_team: attackers_in_zone.append(vehID)
    ...
    def _lock_offline_spawn_pose(self, veh, vehID, isPlayer=False):
        """Поза с _spawn_xyz, не с entity.position (у BigWorld Vehicle он часто 0,0,0)."""
        if veh is None:
            return None
        try:
    ...
    def _showEndBattleStats(self, forced_winner=None, early_exit=False):
        ...
        def _calc_battle_rewards(frags, total_dmg, shots, hits_total, winner_team, player_team, player_descr, killed_ids, spotted_ids):
            try:
                ...
                return int(total_dmg * 0.12) + int(frags) * 40 + len(spotted_ids or []) * 30, \
                       int(total_dmg * 9.0) + int(frags) * 150 + len(spotted_ids or []) * 100
    ...
    def _compute_battle_achievements(self, reason, winner_team):
        """Медали «за бой» ровно по своим условиям (battle_heroes.ACHIEVEMENT_"""
        out = []
        try:
            _av = self.playerAvatar
        ...
        for vehID, _, _ in self.vehicles:
            try:
                ...
                if veh and getattr(veh, 'isStarted', False):
                    try: BigWorld.delShadowEntity(veh)
                    except: pass
                    veh.stopVisual()
                    veh.isStarted = False
            except: pass
        for vehID, _, _ in self.vehicles:
            try: BigWorld.destroyEntity(vehID)
            except: pass

        if self.spaceID is not None:
            try: BigWorld.releaseSpace(self.spaceID)
            except:
                try: BigWorld.clearSpace(self.spaceID)
                except: pass
            self.spaceID = None
...
def _arena_defs_available(arenaTypeID):
    """Проверка scripts/arena_defs/<typeName>.xml через ArenaType.g_list, не строку как id."""
    typeID, typeName = _arena_name_to_id(arenaTypeID)
    if typeName is None:
        return False
    try:
