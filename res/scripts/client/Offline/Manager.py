...
def _resolve_arena_type_id(arenaTypeID):
    """Как TrainingRoom: ArenaType.g_list[id] = name, g_cache.get(id)."""
    try:
...
def _push_battle_results_notification(results):
    """Результаты боя в историю канала (список), без всплывающего «Внимание!»."""
    if not results:
        return
    try:
...
class _MemSection(object):
    """In-memory DataSection for AccountSettings filters (shop/inventory tabs)."""
...
def _flat_ammo_list(shells):
    """InventoryParser / setShellsList: плоский список compactDescr, count, ..."""
    if not shells:
        return []
    first = shells[0]
    out = []
...
def _normalize_loaded_shells(shell_list, gunDescr=None):
    """Снаряды ангара: int-счётчики. Если один тип = maxAmmo пушки"""
...
def _notify_vehicle_inv(inv_id, **parts):
    """Hangar.update: inventory[1][compDescr|shells|eqs|...].has_key(currentId)."""
    try:
        inv1 = {}
...
def _load_economics():
    """Цены слота/обмена/према/свободного XP — как Economics.py из scripts/economics.xml."""
    global _econ_cache
    if _econ_cache:
        return _econ_cache
    result = {
...
def _read_nickname_file():
    """Ник из nickname.txt в корне игры — тот же файл, что уже лежит рядом с клиентом."""
    paths = []
    try:
...
def _vehicle_sell_credits(veh_comp_descr):
    """Цена продажи как Shop.getSellPrice + parseVehicleCompactDescr (не parseIntCompactDescr:"""
...
def _add_premium_vehicle_unlocks(unlocks):
    """В list.xml премиум — цена в золоте, клиент вешает тег 'premium' (vehicles.py)."""
...
def _apply_player_name(player=None):
    """Пишет ник в Account и в шапку common.swf (common.nameResponse)."""
    global _player_name
...
def _grant_vehicle_if_missing(type_name):
    """Исследовать и выдать танк в гараж, если его ещё нет."""
    from items import vehicles as _v
    if not type_name:
        return False
...
def _build_all_vehicle_cds():
    """Все танки игры (не только из ангара) для очереди: строим список"""
...
def _hangar_xp_value():
    """Число в шапке ангара (common.experienceResponse / «Общий опыт»)."""
...
def _emit_server_stats():
    """Люди на сервере / идущие бои — оффлайн-заглушка с живыми целыми числами."""
    try:
...
def _crew_role_level(crew_type):
    """ShopItem.buy: crew_type — индекс tankmanCost (0.6.2.8: 50 / 75 / 100)."""
...
def _shop_items_for_parser(itemTypeIdx, nationIdx):
    """ShopParser.parseVehicles/parseModules: data[0]=prices dict, data[1]=hidden set."""
                    ...
                    def fake_equipEquipments(self, vehicleInvID, equipCompDescrs, callback=None):
                        try:
                            ...
                            for i, cd in enumerate(wanted):
                                ...
                                slots[i] = cd
                            _vehicle_equipments[vehicleInvID] = slots
                            inv_data[6][vehicleInvID] = slots
                            inv_data[5][vehicleInvID] = slots
                            _refresh_player_inventory_items()
                            from CurrentVehicle import g_currentVehicle
                            veh = g_currentVehicle.vehicle
                            if veh and getattr(veh, 'inventoryId', None) == vehicleInvID:
                                try:
                                    veh._InventoryVehicle__equipments = slots
                                except Exception:
                                    pass
                            _save_profile(force=True)
                        except Exception as e:
                            ...
                            if g_currentVehicle.vehicle and \
                               g_currentVehicle.vehicle.inventoryId == inventoryId:
                                if veh_cds:
                                    ...
                                    _CV.g_currentVehicle._CurrentVehicle__vehicle = \
                                        OfflineVehicleWrapper(first)
...
def apply_battle_rewards(earned_credits, earned_xp, veh_inv_id, account_player=None):
    """Зачисляет кредиты и опыт танку после боя. Вызывается из BattleStarter."""
    import BigWorld
    from PlayerEvents import g_playerEvents
    try:
        p = account_player
...
def restore_lobby():
    """Вызывается из BattleStarter для возврата в ангар после боя"""
