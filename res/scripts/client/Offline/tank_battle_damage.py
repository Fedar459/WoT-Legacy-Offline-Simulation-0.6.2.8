# -*- coding: utf-8 -*-
"""Заброневые повреждения модулей и экипажа по typeDescriptor 0.6.2.8."""
...
def crew_stat_factor(vehicle, stat):
    """Множитель характеристики от того, кто из экипажа УБИТ (не просто"""
...
def module_stat_factor(vehicle, stat):
    """Множитель характеристики от состояния МОДУЛЯ (независимо от"""
...
def clamp_vision_factor(factor):
    """Ограничивает множитель обзора снизу - см."""
    try:
        value = float(factor)
...
class Module(object):
    ...
    def apply_hit(self, damage):
        """Безусловно применяет попадание - решение "достанется ли именно"""
    ...
    def repair(self):
        """Авто-ремонт модуля - ПОРТ ИЗ 0.8.2 (device_damage.py):"""
        if self.state == MODULE_STATE_OK:
            return False
        if self.state == MODULE_STATE_DESTROYED:
    ...
    def repair_seconds(self, repair_skill_pct=0.0, has_big_repairkit=False):
        """Секунды до срабатывания авто-ремонта этого модуля - гусеница"""
...
class CrewMember(object):
    ...
    def apply_hit(self, damage):
        """Безусловно применяет попадание (см."""
...
class Vehicle(object):
    """Контейнер модулей и экипажа одного танка."""
...
class DamageResolver(object):
    """Без собственного состояния - один экземпляр можно использовать для"""

    def resolve_hit(self, vehicle, damage, shell_type=ShellType.AP, penetrated=True, hit_component=None):
        """Обрабатывает одно попадание снаряда по танку - в два шага, как и"""
        result = {
            'damage_dealt': 0,
            'module_events': [],
            'crew_events': [],
...
def get_module_sound(module_type, state):
    """Название события для IngameSoundNotifications.play(...) на смену"""
    return _MODULE_SOUNDS.get(module_type, {}).get(state)

def get_crew_kill_sound(role):
    """Название события на гибель конкретной роли экипажа, или None."""
    return _CREW_KILLED_SOUNDS.get(role)

def _comp_health(comp, key=None):
    """(maxHealth, maxRegenHealth) из компонента дескриптора (dict-like,"""
    if comp is None:
        return (None, None)
    target = comp
    if key is not None:
...
def _misc_factor(descr, key):
    """Множитель здоровья устройства от установленного доп."""
    if not key:
        return 1.0
    try:
...
def build_vehicle_from_descriptor(descr, name=None, max_hp=None):
    """Строит объект Vehicle (модули + экипаж-заглушка под штатный расчёт"""
    if max_hp is None:
...
def _build_demo_vehicle(name='Demo Tank', max_hp=380):
    """Пример полностью укомплектованного танка ДЛЯ САМОТЕСТА (не для"""
